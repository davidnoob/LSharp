...
namespace NoobDraven
{
    internal class NoobDraven
    {
        public Spell E;
        public Spell Q;
        ...
        public void Load()
        {
            ...
            Game.PrintChat("<font color=\"#7CFC00\"><b>NoobDraven</b></font> Loaded");
        ...
        private void DrawingOnOnDraw(EventArgs args)
        {
            ...
            var drawAxeLocation = Menu.Item("DrawAxeeRange").IsActive();
            var drawAxeRange = Menu.Item("DrawAARange").IsActive();
                    ...
                    QReticles
                        .Where(
                            x =>
                                ...
                                Menu.Item("CatchAxe").GetValue<Slider>().Value)
            ...
            if (drawAxeRange)
            {
                Render.Circle.DrawCircle(Game.CursorPos, Menu.Item("CatchAxe").GetValue<Slider>().Value,
                    Color.DodgerBlue);
            }
        }
        ...
        {
            if (!Menu.Item("UseItem").IsActive() || !E.IsReady() || !sender.IsValidTarget(E.Range))
            {
                return;
            }
        ...
        {
            var catchOption = Menu.Item("SmathMode").GetValue<StringList>().SelectedIndex;
                ...
                (catchOption == 1 && Orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.None) || catchOption == 2)
            {
                    ...
                    QReticles
                        .Where(
                            x =>
                                ...
                                Menu.Item("CatchR").GetValue<Slider>().Value)
                        .OrderBy(x => x.Position.Distance(Player.ServerPosition))
                        .ThenBy(x => x.Position.Distance(Game.CursorPos))
                        .FirstOrDefault();
                ...
                if (bestReticle != null && bestReticle.Object.Position.Distance(Player.ServerPosition) > 110)
                {
                    ...
                    if (Menu.Item("Don't CatchR Turret").IsActive())
                    {
        ...
        private void Combo()
        {
            ...
            var useQ = Menu.Item("Use Q Combo").IsActive();
            var useW = Menu.Item("Use W Combo").IsActive();
            var useE = Menu.Item("Use E Combo").IsActive();
            var useR = Menu.Item("Use R Combo").IsActive();

            if (useQ && QCount < Menu.Item("MaxAxe").GetValue<Slider>().Value - 1 && Q.IsReady() &&
            ...
            if (useW && W.IsReady() && ManaPercent > Menu.Item("Use W").GetValue<Slider>().Value)
            {
                if (Menu.Item("W Setting").IsActive())
                {
                    W.Cast();
                }
                else
        ...
        private void LaneClear()
        {
            ...
            var useW = Menu.Item("UseWLaneClear").IsActive();
            var useE = Menu.Item("UseELaneClear").IsActive();

            if (ManaPercent < Menu.Item("LaneClearManage").GetValue<Slider>().Value)
            {
                return;
            }
            ...
            if (useW && W.IsReady() && ManaPercent > Menu.Item("Use W").GetValue<Slider>().Value)
            {
                if (Menu.Item("Use W Setting").IsActive())
                {
                    W.Cast();
                }
                else
        ...
        private void Harass()
        {
            ...
            if (Menu.Item("Use E Harass").IsActive() && E.IsReady())
            {
                E.Cast(target);
            }
        }
        ...
        {
            Menu = new Menu("NoobDraven", "Draven", true);
            ...
            // Lane Clear
            var laneClearMenu = new Menu("Lane Clear", "waveclear");
            ...
            laneClearMenu.AddItem(new MenuItem("UseELaneClear", "Use E").SetValue(false));
            laneClearMenu.AddItem(new MenuItem("LaneClearManaPercent", "Mana Percent").SetValue(new Slider(50)));
            ...
            axeMenu.AddItem(new MenuItem("MaxAxe", "Maximum Axes").SetValue(new Slider(2, 1, 3)));
            ...
            miscMenu.AddItem(new MenuItem("Use WS etting", "Use W Instantly(When Available)").SetValue(false));
            miscMenu.AddItem(new MenuItem("Use E Gapcloser", "Use E on Gapcloser").SetValue(true));
            miscMenu.AddItem(new MenuItem("Use E Interrupt", "Use E to Interrupt").SetValue(true));
            miscMenu.AddItem(new MenuItem("Use W ManaPercent", "Use W Mana Percent").SetValue(new Slider(50)));
            miscMenu.AddItem(new MenuItem("Use W Slow", "Use W if Slowed").SetValue(true));
